...
| Cross-encoder reranker | `image/optimus_server.py` | **Superseded.** v2's reranker stack is locked: Nomic CodeRankEmbed (dense retrieval) + ColBERTv2 via RAGatouille (late-interaction rerank), per `docs/decisions/secure-singleton-mcp-baseline.md`. The dual-CE-vs-single-CE question is retired; the late-interaction architecture replaces it. No salvage from v1's CE wrapper code beyond reference value. |
| spaCy pipeline | `image/optimus_server.py` | Salvage; retain/drop call gated on spike-1 evidence per CHARTER Decision 5. Scope narrowed to "preprocessing layer ahead of the locked Nomic + ColBERTv2 pipeline," not a substitute for it. |
